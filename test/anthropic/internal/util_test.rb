...
class Anthropic::Test::UtilDataHandlingTest < Minitest::Test
  def test_left_map
    assert_pattern do
      Anthropic::Internal::Util.deep_merge({a: 1}, nil) => nil
    end
  end

  def test_right_map
    assert_pattern do
      Anthropic::Internal::Util.deep_merge(nil, {a: 1}) => {a: 1}
    end
  end

  def test_disjoint_maps
    assert_pattern do
      Anthropic::Internal::Util.deep_merge({b: 2}, {a: 1}) => {a: 1, b: 2}
    end
  end

  def test_overlapping_maps
    assert_pattern do
      Anthropic::Internal::Util.deep_merge({b: 2, c: 3}, {a: 1, c: 4}) => {a: 1, b: 2, c: 4}
    end
  end

  def test_nested
    assert_pattern do
      Anthropic::Internal::Util.deep_merge({b: {b2: 1}}, {b: {b2: 2}}) => {b: {b2: 2}}
    end
  end

  def test_nested_left_map
    assert_pattern do
      Anthropic::Internal::Util.deep_merge({b: {b2: 1}}, {b: 6}) => {b: 6}
    end
  end
  ...
  def test_dig
    assert_pattern do
      Anthropic::Internal::Util.dig(1, nil) => 1
      Anthropic::Internal::Util.dig({a: 1}, :b) => nil
      Anthropic::Internal::Util.dig({a: 1}, :a) => 1
      Anthropic::Internal::Util.dig({a: {b: 1}}, [:a, :b]) => 1

      Anthropic::Internal::Util.dig([], 1) => nil
      Anthropic::Internal::Util.dig([nil, [nil, 1]], [1, 1]) => 1
      Anthropic::Internal::Util.dig({a: [nil, 1]}, [:a, 1]) => 1
...
class Anthropic::Test::UtilUriHandlingTest < Minitest::Test
  def test_parsing
    %w[
      ...
      parsed = Anthropic::Internal::Util.parse_uri(url)
      unparsed = Anthropic::Internal::Util.unparse_uri(parsed).to_s

      assert_equal(url, unparsed)
      assert_equal(parsed, Anthropic::Internal::Util.parse_uri(unparsed))
    end
  end
  ...
  def test_joining
    cases = [
      [
        "h://a.b/c?d=e",
        "h://nope/ignored",
        Anthropic::Internal::Util.parse_uri("h://a.b/c?d=e")
      ],
      [
        "h://a.b/c?d=e",
        "h://nope",
    ...
    cases.each do |expect, lhs, rhs|
      assert_equal(
        URI.parse(expect),
        Anthropic::Internal::Util.join_parsed_uri(
          Anthropic::Internal::Util.parse_uri(lhs),
          rhs
        )
      )
    end
  ...
  def test_joining_queries
    ...
    cases.each do |path, expected|
      assert_equal(
        URI.parse(expected),
        Anthropic::Internal::Util.join_parsed_uri(
          Anthropic::Internal::Util.parse_uri(base_url),
          {path: path}
        )
      )
    end
...
class Anthropic::Test::UtilFormDataEncodingTest < Minitest::Test
  ...
  def test_encoding_length
    headers, = Anthropic::Internal::Util.encode_content(
  ...
  def test_file_encode
    ...
    fileinput = Anthropic::Internal::Type::Converter.dump(Anthropic::Internal::Type::FileInput, "abc")
  ...
  def test_hash_encode
    ...
    cases = {
      ...
      {pathname: Anthropic::FilePart.new(Pathname(__FILE__))} => {"pathname" => -> { _1.read in /^class Anthropic/ }}
    }
    cases.each do |body, testcase|
      encoded = Anthropic::Internal::Util.encode_content(headers, body)
      cgi = FakeCGI.new(*encoded)
...
class Anthropic::Test::UtilIOAdapterTest < Minitest::Test
  def test_copy_read
    cases = {
      StringIO.new("abc") => "abc",
    ...
    cases.each do |input, expected|
      io = StringIO.new
      # rubocop:disable Lint/EmptyBlock
      adapter = Anthropic::Internal::Util::ReadIOAdapter.new(input) {}
      # rubocop:enable Lint/EmptyBlock
  ...
  def test_copy_write
    ...
    cases.each do |input, expected|
      enum = Anthropic::Internal::Util.writable_enum do |y|
...
class Anthropic::Test::UtilFusedEnumTest < Minitest::Test
  def test_rewind_closing
    ...
    fused = Anthropic::Internal::Util.fused_enum(enum, external: true) do
      touched = true
      loop { enum.next }
    end
    Anthropic::Internal::Util.close_fused!(fused)
  ...
  def test_thread_interrupts
    ...
    fused_1 = Anthropic::Internal::Util.fused_enum(enum, external: true) { loop { enum.next } }
    fused_2 = Anthropic::Internal::Util.chain_fused(fused_1) { fused_1.each(&_1) }
  ...
  def test_closing
    arr = [1, 2, 3]
    once = 0
    fused = Anthropic::Internal::Util.fused_enum(arr.to_enum) do
      once = once.succ
    end
  ...
  def test_rewind_chain
    once = 0
    fused = Anthropic::Internal::Util.fused_enum([1, 2, 3].to_enum) do
      once = once.succ
    end
      .lazy
      .map(&:succ)
  ...
  def test_external_iteration
    iter = [1, 2, 3].to_enum
    first = iter.next
    fused = Anthropic::Internal::Util.fused_enum(iter, external: true)
  ...
  def test_closed_fused_extern_iteration
    ...
    fused = Anthropic::Internal::Util.fused_enum(enum)
    first = fused.next
  ...
  def test_close_fused_sse_chain
    ...
    fused_1 = Anthropic::Internal::Util.fused_enum(enum)
    fused_2 = Anthropic::Internal::Util.decode_lines(fused_1)
    fused_3 = Anthropic::Internal::Util.decode_sse(fused_2)

    assert_equal(0, taken)
    Anthropic::Internal::Util.close_fused!(fused_3)
    assert_equal(0, taken)
  end
end
...
class Anthropic::Test::UtilSseTest < Minitest::Test
  def test_decode_lines
    ...
    cases.each do |enum, expected|
      eols.each do |eol|
        lines = Anthropic::Internal::Util.decode_lines(enum.map { _1.gsub("\n", eol) })
  ...
  def test_mixed_decode_lines
    ...
    cases.each do |enum, expected|
      lines = Anthropic::Internal::Util.decode_lines(enum)
  ...
  def test_decode_sse
    ...
    cases.each do |name, test_cases|
      test_cases.each do |input, expected|
        actual = Anthropic::Internal::Util.decode_sse(input).map(&:compact)
