...
#
# Run: ANTHROPIC_LIVE=1 bundle exec ruby test/anthropic/helpers/bedrock/mantle_client_live_test.rb
class Anthropic::Test::BedrockMantleClientLiveTest < Minitest::Test
  ...
  def send_mantle_message(client)
  ...
  def test_live_mantle_api_key
    skip_unless_live
    api_key = ENV["AWS_BEARER_TOKEN_BEDROCK"] || ENV["ANTHROPIC_AWS_API_KEY"]
    raise "required env var AWS_BEARER_TOKEN_BEDROCK or ANTHROPIC_AWS_API_KEY is not set" if api_key.to_s.empty?
    require_env("AWS_REGION")

    client = Anthropic::BedrockMantleClient.new(api_key: api_key)
    send_mantle_message(client)
  end

  def test_live_mantle_sigv4_explicit_creds
    skip_unless_live
  ...
  def test_live_mantle_sigv4_aws_profile
    skip_unless_live
    profile = ENV.fetch("ANTHROPIC_MANTLE_AWS_PROFILE", "anthropic-mantle")
    require_env("AWS_REGION")
    ...
    original_mantle_key = ENV.delete("AWS_BEARER_TOKEN_BEDROCK")
    original_aws_key = ENV.delete("ANTHROPIC_AWS_API_KEY")
    original_api_key = ENV.delete("ANTHROPIC_API_KEY")

    begin
      client = Anthropic::BedrockMantleClient.new(aws_profile: profile)
      send_mantle_message(client)
    ensure
