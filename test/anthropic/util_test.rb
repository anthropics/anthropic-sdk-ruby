...
class Anthropic::Test::UtilDataHandlingTest < Minitest::Test
  def test_left_map
    assert_pattern do
      Anthropic::Util.deep_merge({a: 1}, nil) => nil
    end
  end

  def test_right_map
    assert_pattern do
      Anthropic::Util.deep_merge(nil, {a: 1}) => {a: 1}
    end
  end
  ...
  def test_overlapping_maps
    assert_pattern do
      Anthropic::Util.deep_merge({b: 2, c: 3}, {a: 1, c: 4}) => {a: 1, b: 2, c: 4}
    end
  end

  def test_nested
    assert_pattern do
      Anthropic::Util.deep_merge({b: {b2: 1}}, {b: {b2: 2}}) => {b: {b2: 2}}
    end
  end

  def test_nested_left_map
    assert_pattern do
      Anthropic::Util.deep_merge({b: {b2: 1}}, {b: 6}) => {b: 6}
    end
  end

  def test_omission
    merged = Anthropic::Util.deep_merge(
      {b: {b2: 1, b3: {c: 4, d: 5}}},
  ...
  def test_dig
    assert_pattern do
      Anthropic::Util.dig(1, nil) => 1
      Anthropic::Util.dig({a: 1}, :b) => nil
      Anthropic::Util.dig({a: 1}, :a) => 1
      Anthropic::Util.dig({a: {b: 1}}, [:a, :b]) => 1

      Anthropic::Util.dig([], 1) => nil
      Anthropic::Util.dig([nil, [nil, 1]], [1, 1]) => 1
      Anthropic::Util.dig({a: [nil, 1]}, [:a, 1]) => 1
      Anthropic::Util.dig([], 1.0) => nil

      Anthropic::Util.dig(Object, 1) => nil
      Anthropic::Util.dig([], 1.0, 2) => 2
      Anthropic::Util.dig([], 1.0) { 2 } => 2
    end
  end
end
...
class Anthropic::Test::UtilFormDataEncodingTest < Minitest::Test
  class FakeCGI < CGI
    def initialize(headers, io)
      ...
      @io = Anthropic::Util::ReadIOAdapter.new(io) {}
  ...
  def test_file_encode
    ...
    cases.each do |body, val|
      encoded = Anthropic::Util.encode_content(headers, body)
...
class Anthropic::Test::UtilIOAdapterTest < Minitest::Test
  def test_copy_read
    ...
    cases.each do |input, expected|
      io = StringIO.new
      # rubocop:disable Lint/EmptyBlock
      adapter = Anthropic::Util::ReadIOAdapter.new(input) {}
      # rubocop:enable Lint/EmptyBlock
  ...
  def test_copy_write
    ...
    cases.each do |input, expected|
      enum = Anthropic::Util.writable_enum do |y|
...
class Anthropic::Test::UtilFusedEnumTest < Minitest::Test
  def test_closing
    arr = [1, 2, 3]
    once = 0
    fused = Anthropic::Util.fused_enum(arr.to_enum) do
      once = once.succ
    end
  ...
  def test_rewind_chain
    once = 0
    fused = Anthropic::Util.fused_enum([1, 2, 3].to_enum) do
      once = once.succ
    end
      .lazy
      .map(&:succ)
  ...
  def test_external_iteration
    it = [1, 2, 3].to_enum
    first = it.next
    fused = Anthropic::Util.fused_enum(it, external: true)
  ...
  def test_close_fused
    ...
    Anthropic::Util.close_fused!(fused)
  ...
  def test_closed_fused_extern_iteration
    ...
    fused = Anthropic::Util.fused_enum(enum)
    first = fused.next
  ...
  def test_close_fused_sse_chain
    ...
    fused_1 = Anthropic::Util.fused_enum(enum)
    fused_2 = Anthropic::Util.decode_lines(fused_1)
    fused_3 = Anthropic::Util.decode_sse(fused_2)

    assert_equal(0, taken)
    Anthropic::Util.close_fused!(fused_3)
    assert_equal(0, taken)
  end
end
...
class Anthropic::Test::UtilSseTest < Minitest::Test
  def test_decode_lines
    ...
    cases.each do |enum, expected|
      eols.each do |eol|
        lines = Anthropic::Util.decode_lines(enum.map { _1.gsub("\n", eol) })
  ...
  def test_mixed_decode_lines
    ...
    cases.each do |enum, expected|
      lines = Anthropic::Util.decode_lines(enum)
  ...
  def test_decode_sse
    ...
    cases.each do |name, test_cases|
      test_cases.each do |input, expected|
        actual = Anthropic::Util.decode_sse(input).map(&:compact)
