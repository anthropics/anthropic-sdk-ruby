...
class Anthropic::Test::BaseModelTest < Minitest::Test
  ...
  def test_basic
    assert(E1.is_a?(Anthropic::Converter))
    assert(A1.is_a?(Anthropic::Converter))
  end

  def test_basic_coerce
    assert_pattern do
      Anthropic::Converter.coerce(A1, [1.0, 2.0, 3.0]) => [1, 2, 3]
    end

    assert_pattern do
      Anthropic::Converter.coerce(A2, %w[a b c]) => [:a, :b, "c"]
    end
  end

  def test_basic_dump
    assert_pattern do
      Anthropic::Converter.dump(A1, [1.0, 2.0, 3.0]) => [1, 2, 3]
    end

    assert_pattern do
      Anthropic::Converter.dump(A2, %w[a b c]) => %w[a b c]
    end
  end
  ...
  def test_primitive_try_strict_coerce
    ...
    cases.each do |test, expect|
      type, input = test
      assert_pattern do
        Anthropic::Converter.try_strict_coerce(type, input) => ^expect
      end
    end
  end
  ...
  def test_basic_enum_try_strict_coerce
    ...
    cases.each do |input, expect|
      assert_pattern do
        Anthropic::Converter.try_strict_coerce(E1, input) => ^expect
      end
    end
  end
  ...
  def test_basic_array_try_strict_coerce
    ...
    cases.each do |input, expect|
      assert_pattern do
        Anthropic::Converter.try_strict_coerce(A1, input) => ^expect
      end
    end
  end
  ...
  def test_nested_array_try_strict_coerce
    ...
    cases.each do |input, expect|
      assert_pattern do
        Anthropic::Converter.try_strict_coerce(A2, input) => ^expect
      end
    end
  end
  ...
  def test_basic_model_coerce
    ...
    cases.each do |input, expect|
      assert_pattern do
        Anthropic::Converter.coerce(M2, input) => ^expect
      end
    end
  end
  ...
  def test_basic_model_dump
    ...
    cases.each do |input, expect|
      assert_pattern do
        Anthropic::Converter.dump(M2, input) => ^expect
      end
    end
  end
  ...
  def test_basic_model_try_strict_coerce
    ...
    cases.each do |input, expect|
      assert_pattern do
        Anthropic::Converter.try_strict_coerce(M1, input) => ^expect
      end
    end
  end
  ...
  def test_nested_model_dump
    ...
    models.product(inputs).each do |model, input|
      assert_pattern do
        Anthropic::Converter.dump(model, input) => {a: now, renamed: "a", c: [1, 2, 3]}
      end
    end
  end
  ...
  class M4 < M2
    required :c, M1
    required :d, Anthropic::ArrayOf[M4]
  ...
  def test_basic_union
    assert(U1.is_a?(Anthropic::Converter))
  ...
  def test_basic_discriminated_union_coerce
    ...
    cases.each do |input, expect|
      assert_pattern do
        Anthropic::Converter.coerce(U1, input) => ^expect
      end
    end
  end
  ...
  def test_basic_discriminated_union_dump
    ...
    cases.each do |input, expect|
      assert_pattern do
        Anthropic::Converter.dump(U1, input) => ^expect
      end
    end
  end
  ...
  def test_basic_undifferentiated_union_try_strict_coerce
    ...
    cases.each do |input, expect|
      assert_pattern do
        Anthropic::Converter.try_strict_coerce(U2, input) => ^expect
      end
    end
  end

  class C1 < Anthropic::BaseModel
    required :a, const: :a
  ...
  def test_basic_eql
    assert_equal(Anthropic::Unknown, Anthropic::Unknown)
    refute_equal(Anthropic::Unknown, Anthropic::BooleanModel)
    assert_equal(Anthropic::BooleanModel, Anthropic::BooleanModel)
